...
const props = withDefaults(
  ...
  { rateUnit: 'rps', showVu: false, vuData: () => [], zoomEnabled: true, visibleRange: null },
)
...
const { selection } = useLineChart({
  ...
  render: () => {
    ...
    const datasets: unknown[] = props.series.map((s, i) => {
      ...
      return {
        ...
        data: props.axis.map((b) => {
          ...
          return point && point.calls > 0 ? point.throughput * factor : null
        }),
    ...
    return {
      labels: labels.value,
      datasets,
      yTitle: rateUnitLabel.value,
      formatTick: formatValue,
      tooltipLabel: makeTooltipLabel(formatValue, ` ${rateUnitLabel.value}`),
      scales,
    }
  },
  select: {
